...
export default function Home() {
  ...
  const { data: articles, isLoading, isError, isFetching, dataUpdatedAt } = useQuery<NewsArticle[]>({
    queryKey: ["/api/news", { category, search, sources: selectedSources, dates: selectedDates }],
    queryFn: async () => {
  ...
  const hasActiveFilters = selectedSources.length > 0 || selectedDates.length > 0;
      ...
      <div className="border-b">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <DateFilter selectedDates={selectedDates} onDatesChange={setSelectedDates} />
              <SourceFilter selectedSources={selectedSources} onSourcesChange={setSelectedSources} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {search
                ...
                ? "No articles found for the selected date(s). Try a different range."
